...
typedef struct bstnode_t {
  int key;
  int par;
  struct bstnode_t *left, *right, *sibling;
} BSTNode;
...
BSTNode* __bst__createNode(int value, BSTNode *node, int prevval) {
  // printf("flag4\n");
...
BSTNode* __bst__insert(BSTNode *root, BSTNode *node, int value, int prevval) {
  if (root == NULL)
  {
    // printf("flag2\n");
    return __bst__createNode(value, node, prevval);
  }
  ...
  if (value < root->key)
  {
    ...
    root->left = __bst__insert(root->left, node, value, prevval);
    // printf("flag6\n");

    if (root->right != NULL)
    {
      if (root->right->sibling == NULL)
  ...
  else if (value > root->key)
  {
    ...
    root->right = __bst__insert(root->right, node, value, prevval);

    if (root->left != NULL)
    {
      if (root->left->sibling == NULL)
...
void __bst__preorder(BSTNode *root) {
  if (root) {
    if (root->par != 0)
      if (root->sibling != NULL)
        printf("%d..%d %d >> ", root->key, root->par, root->sibling->key);
      else
...
void bst_insert(BST *bst, int value, int prevval) {
  if (!bst_find(bst, value)) {
    BSTNode *node = NULL;
    bst->_root = __bst__insert(bst->_root, node, value, prevval);
    bst->_size++;
  }
}
