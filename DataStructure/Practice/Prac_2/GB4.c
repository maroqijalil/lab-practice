...
void bst_insert(BST *bst, int value) {
  if (!bst_find(bst, value)) {
    ...
    if (bst->_root == NULL)
    {
      bst->_root = __bst__createNode(value, prevval, level);
      return;
    }
    ...
    if (node == NULL)
      node = __bst__createNode(value, prevval, level);
    else if (node->key < value)
      node->right = __bst__createNode(value, prevval, level);
    else if (node->key > value)
      node->left = __bst__createNode(value, prevval, level);
    else if (node->key == value)
      return;

    root = __bst__createNode(value, prevval, level);
    BSTNode *newNode = root;

    printf("%d\n", node->key);

    if (bst->_pqueue_size < level)
      bst->_pqueue = (pQue_bst*)malloc(sizeof(pQue_bst) * level);

    while (bst->_pqueue_size < level)
    {
      pque_init((pQue_bst*)&bst->_pqueue[bst->_pqueue_size]);
      bst->_pqueue_size += 1;
      printf("%d\n", bst->_pqueue_size);
    }
    
    pQue_bst *pqueue;
    *pqueue = bst->_pqueue[level-1];

    BSTNode *temp = pqueue->_head;
    pqueue->_size++;

    printf("lev %d siz %d\n", level, bst->_pqueue_size);
    ...
    {
      if (value < pqueue->_head->key) {
        newNode->r_sibling = pqueue->_head;
        pqueue->_head->l_sibling = newNode;
        pqueue->_head = newNode;
      }
      else
      {
        pqueue->_head->r_sibling = newNode;
        newNode->l_sibling = pqueue->_head;
    ...
    else
    {
      if (value < pqueue->_head->key) {
        ...
        {
          if (pqueue->_tail->key < newNode->key)
          {
            // printf("flag 1\n");
            newNode->l_sibling = pqueue->_tail;
            pqueue->_tail->r_sibling = newNode;
            pqueue->_tail = newNode;
          }
          else
          {
            // printf("flag 2\n");
            newNode->l_sibling = pqueue->_tail->l_sibling;
            pqueue->_tail->l_sibling->r_sibling = newNode;
            pqueue->_tail->l_sibling = newNode;
            newNode->r_sibling = pqueue->_tail;
          }
        }
        else
        {
