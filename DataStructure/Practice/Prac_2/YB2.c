...
void printminmax(BSTNode* root, int level, int *num, int *lev) 
{
	if (root == NULL) 
		return; 
	if (level == 1)
  {
	...
	{ 
		printminmax(root->left, level-1, num, lev); 
		printminmax(root->right, level-1, num, lev); 
	} 
} 
