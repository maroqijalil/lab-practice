...
AVLNode* _rightRotate(AVLNode* pivotNode){
    ...
    pivotNode->l_height = 1 + _getHeight(pivotNode->left);
    pivotNode->r_height = 1 + _getHeight(pivotNode->right);
    
    if (pivotNode->left != NULL)
      pivotNode->l_child = 1 + pivotNode->left->num_child;
    else
      pivotNode->l_child = 0;

    if (pivotNode->right != NULL)
      pivotNode->r_child = 1 + pivotNode->right->num_child;
    else
      pivotNode->r_child = 0;
    
    pivotNode->num_child = pivotNode->r_child + pivotNode->l_child;

    newParrent->height=_max(_getHeight(newParrent->left),
                       _getHeight(newParrent->right))+1;

    newParrent->l_height = 1 + _getHeight(newParrent->left);
    newParrent->r_height = 1 + _getHeight(newParrent->right);

    if (newParrent->left != NULL)
      newParrent->l_child = 1 + newParrent->left->num_child;
    else
      newParrent->l_child = 0;

    if (newParrent->right != NULL)
      newParrent->r_child = 1 + newParrent->right->num_child;
    else
      newParrent->r_child = 0;
    
    newParrent->num_child = newParrent->r_child + newParrent->l_child;
...
AVLNode* _insert_AVL(AVL *avl, AVLNode* node, int value, int ap){
    ...
    node->num_child = node->r_child + node->l_child;
    // printf("%d n:%d l:%d r:%d\n", node->data,\
    //       node->num_child, node->l_child, node->r_child);
