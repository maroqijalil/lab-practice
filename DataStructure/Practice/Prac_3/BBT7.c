    ...
    if (root)
    {
        ...
        if (root->data != rd)
        {
          ...
          *ur += 1;
          ...
          // printf("p1: ");
          // printCharacter(arr[root->data][0]);
          // printf("p2: ");
          // printCharacter(arr[root->data + 1][0]);
          // printf("%d %d\n", arr[root->data][1], arr[root->data + 1][1]);
          
          if (arr[root->data][3] == 0)
          {
            arr[root->data][3] = 1;
            arr[root->data][2] = root->data + 1;
          }

          if (arr[root->data + 1][3] == 0)
          {
            arr[root->data + 1][3] = 1;
            arr[root->data + 1][2] = root->data;
          }

          if ((arr[root->data][1] = arr[root->data][1] - arr[root->data + 1][1]) < 0)
          {
            arr[root->data][1] = abs(arr[root->data][1]);
            arr[root->data][0] = arr[root->data + 1][0];
          }

          arr[root->data + 1][1] = arr[root->data][1];
          arr[root->data + 1][0] = arr[root->data][0];

          arr[arr[root->data + 1][2]][1] = arr[root->data][1];
          arr[arr[root->data + 1][2]][0] = arr[root->data][0];

          // (arr + (root->data + 1)) = (arr + root->data);
          int i = 0;
          for (; i < n; i++)
          {
            if (arr[i][0] == arr[root->data][0] || arr[i][0] == arr[root->data + 1][0]\
              || arr[i][0] == arr[arr[root->data + 1][2]][0] || arr[i][0] == arr[arr[root->data][2]][0] )
            {
              arr[i][0] = arr[root->data][0];
              arr[i][1] = arr[root->data][1];
            }
          }

          // printf("p3: ");
          // printCharacter(arr[arr[root->data + 1][2]][0]);

          printStat(arr[root->data][0], arr[root->data][1], *ur);
          // printf("%d\n", arr[root->data][0]);
          *last = root->data;
        }
        else if (root->data == rd)
        {
          // printf("%d\n", *last);
          root->data -= 1;
          ...
          // printf("p2: ");
          // printCharacter(arr[root->data + 1][0]);
          // printf("%d %d\n", arr[root->data][1], arr[*last][1]);

          if ((arr[root->data][1] = arr[root->data][1] - arr[*last][1]) < 0)
          {
            arr[root->data][1] = abs(arr[root->data][1]);
            arr[root->data][0] = arr[*last][0];
          }

          arr[*last][1] = arr[root->data][1];
          arr[*last][0] = arr[root->data][0];
