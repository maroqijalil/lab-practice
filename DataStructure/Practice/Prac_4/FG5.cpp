    ...
    for (j = 0; j < 5; j++)
    {
      if (dist[i][j].size() > 0)
      {
        for (k = 0; k < dist[i][j].size(); k++)
        {
          ...
          key = way_adj(adj, place[ind][0], place[ind][1], &s[ind], matrix);

          if (key)
          {
            // printf("print\n");
            printf("->%d %lu\n", s[ind].top(), s[ind].size());
          ...
          else
          {
            for (l = 0; l < 25; l++)
              matrix2[l] = matrix[l];
            dist[i][j].push_back(ind);
            for (l = 0; l < prev_ind.size(); l++)
            {
              dist[i][j].push_back(prev_ind[l]);
            }
          }

          prev_ind.push_back(ind);
        }
      }
    }
  }
